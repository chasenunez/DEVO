...
def infer_column_type(values: List[str], missing_values: set) -> str:
    ...
    """
    pruned = [
        v.strip() for v in values if v is not None and v.strip() not in missing_values
    ]
    if not pruned:
        return "string"
...
def load_rows_with_frictionless(
...
def build_frictionless_schema(
    header: List[str], col_infos: List[Dict[str, Any]], missing_values: List[str]
) -> Dict[str, Any]:
    schema = {
        "fields": [],
...
def build_fields_section(
    header: List[str], col_infos: List[Dict[str, Any]], field_delimiter: str
) -> List[str]:
    ...
    min_vals = [
        c.get("min", "") if c.get("min", "") is not None else "" for c in col_infos
    ]
    max_vals = [
        c.get("max", "") if c.get("max", "") is not None else "" for c in col_infos
    ]
...
def make_icsv_from_csv(
    ...
    header, rows, row_count = load_rows_with_frictionless(
        infile, delimiter=user_delimiter
    )
    ...
    if nodata_override is not None:
        ...
        nodata_value = (
            max(placeholder_counts.items(), key=lambda x: x[1])[0]
    ...
    for i, name in enumerate(header):
        ...
        pruned = [
            v for v in col_values if v not in ENRICHMENT_COMMON_MISSING and v != ""
        ]
        ...
        missing_count = sum(
            1 for v in col_values if v in ENRICHMENT_COMMON_MISSING or v == ""
        )
        info["missing_count"] = missing_count
        info["description"] = ""
        col_infos.append(info)

    schema = build_frictionless_schema(
        header, col_infos, list(ENRICHMENT_COMMON_MISSING)
    )
