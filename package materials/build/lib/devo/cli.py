...
def main(argv: Optional[list[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="devo", description="DEVO: Data Enrichment and Validation Operator"
    )
    p.add_argument("infile", help="Input CSV or iCSV file")
    p.add_argument(
        "--config", "-c", help="Path to config file (YAML or JSON)", default=None
    )
    p.add_argument(
        "--delimiter",
        "-d",
        help="Force input delimiter (autodetect otherwise)",
        default=None,
    )
    p.add_argument("--nodata", help="Force nodata placeholder value", default=None)
    p.add_argument(
        "--app", help="Optional application profile for iCSV firstline", default=None
    )
    p.add_argument(
        "--schema-out", help="Path to write inferred schema JSON", default=None
    )
    ...
    if infile.suffix.lower() == ".csv":
        # Build sensible defaults using Path helpers
        out_icsv = args.out or cfg.get("out_icsv") or str(infile.with_suffix(".icsv"))
        # Create schema name by inserting "_schema" before the final suffix
        out_schema = args.schema_out or cfg.get("schema_out") or str(infile.with_name(infile.stem + "_schema.json"))
