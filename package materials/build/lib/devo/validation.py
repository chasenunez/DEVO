...
def check_metadata(
    metadata: Dict[str, str], fields_meta: Dict[str, List[str]]
) -> List[str]:
    errors: List[str] = []
    ...
    if "fields" not in fields_meta:
        ...
        for key, values in fields_meta.items():
            if key != "fields" and values and len(values) != num:
                errors.append(
                    f"Inconsistent count in '{key}': expected {num}, found {len(values)}"
                )
    return errors
...
def build_schema_from_metadata(fields_meta: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    schema: Dict[str, Any] = {
        "fields": [],
        "missingValues": ["", "NA", "N/A", "null", "-999", "-999.0"],
    }
    ...
    for i, name in enumerate(fields):
        ...
        if i < len(min_vals) and min_vals[i]:
            try:
                constraints["minimum"] = (
                    float(min_vals[i]) if "." in min_vals[i] else int(min_vals[i])
                )
            except Exception:
        ...
        if i < len(max_vals) and max_vals[i]:
            try:
                constraints["maximum"] = (
                    float(max_vals[i]) if "." in max_vals[i] else int(max_vals[i])
                )
            except Exception:
...
def _format_report(report) -> str:
    ...
    try:
        ...
        for rownum, fieldnum, code, message in report.flatten(
            ["rowNumber", "fieldNumber", "code", "message"]
        ):
            suggestion = ERROR_SUGGESTIONS.get(code, ERROR_SUGGESTIONS["default"])
            lines.append(
                f"  Row {rownum or '?'}, Col {fieldnum or '?'} [{code}]: {message}"
            )
...
def validate_icsv(infile: str, out_report: Optional[str] = None) -> Tuple[bool, str]:
    ...
    if meta_errors:
        ...
        with open(out_report, "w", encoding="utf-8") as rf:
            rf.write(
                "Validation aborted due to metadata errors. See metadata report.\n"
            )
